...
public class BuddiController {
	 ...
	 * @PostMapping("/login")
	 * 
	 * @ResponseBody public Map<String, Boolean> login(BuddiUserVO vo, Model model)
	 * { Map<String, Boolean> map = new HashMap<>(); boolean ok = svc.login(vo);
	 * if(ok) { model.addAttribute("uid", vo.getUid()); // 세션에 저장 } map.put("ok",
	 * ok); return map; }
	 * 
	 * @GetMapping("/logout") public String logout(SessionStatus status) {
	 * status.setComplete(); return "redirect:/user/login"; //
	 ...
	 * Model model) { if(uid==null) { return "redirect:/user/login"; // 로그인 폼으로 ...
	 * }else { List<User> list = svc.getList(); model.addAttribute("list", list);
	 * return "user/userList"; } }
	 * 
	 * @GetMapping("/add") public String userAddForm() { return "user/addForm"; }
	 * 
	 * @PostMapping("/add")
	 * 
	 * @ResponseBody public Map<String, Boolean> addUser(User user) { boolean added
	 * = svc.add(user); Map<String, Boolean> map = new HashMap<>(); map.put("added",
	 * added); return map; }
	 * 
	 * @GetMapping("/detail") public String detail(@RequestParam String userid,
	 * Model model) { User user = svc.detail(userid); model.addAttribute("user",
	 * user); return "user/detail"; }
	 * 
